...
public class TCPFileReceiveHandler extends  Thread {
    ...
    public void shutdown() {
        ...
        running = false;
        try {
            logger.log(Level.INFO,"[" + TCPFileReceiveHandler.currentThread().getId() + " | " + TCPFileReceiveHandler.currentThread().getName() + "] Closing TCPFileReceive Socket");
            serverSocket.close();
        } catch (IOException e) {
            e.printStackTrace();
            logger.log(Level.SEVERE,"[" + TCPFileReceiveHandler.currentThread().getId() + " | " + TCPFileReceiveHandler.currentThread().getName() + "] Something went wrong while trying to close TCPFileReceive Socket");
        }
    }
}
