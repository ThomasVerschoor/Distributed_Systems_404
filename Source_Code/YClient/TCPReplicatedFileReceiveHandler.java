...
public class TCPReplicatedFileReceiveHandler extends Thread{
    ...
    public void shutdown() {
        System.out.println("Quiting TCPFileReceiveHandler...");
        running = false;
        try {
            logger.log(Level.INFO,"[" + TCPReplicatedFileReceiveHandler.currentThread().getId() + " | " + TCPReplicatedFileReceiveHandler.currentThread().getName() + "] Closing TCPReplicatedFileReceive Socket");
            serverSocket.close();
        } catch (IOException e) {
            e.printStackTrace();
            logger.log(Level.SEVERE,"[" + TCPReplicatedFileReceiveHandler.currentThread().getId() + " | " + TCPReplicatedFileReceiveHandler.currentThread().getName() + "] Something went wrong while trying to close TCPReplicatedFileReceive Socket");
        }
    }
}
