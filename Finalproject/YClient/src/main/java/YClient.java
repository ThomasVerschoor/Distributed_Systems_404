...
public class YClient {
    ...
    private static boolean runningRep = false;
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("problem with logger on linux");
        }
        ...
        //Discovery & Bootstrap
        try {
            System.out.println("Starting Discovery and Bootstrap...");
        ...
        while (running) {
            ...
            switch (command) {
                ...
                default:
                    //Check if all ports are ready to receive
                    if (!UDPHandler.isAlive() || !UDPMultiHandler.isAlive() || !TCPFileReceive.isAlive() || !TCPReplicatedFileReceive.isAlive()) {
                        if (!UDPHandler.isAlive()) {
                            UDPHandler.start();
                        }
                        else if (!UDPMultiHandler.isAlive()) {
                            UDPMultiHandler.start();
                        }
                        else if (!TCPFileReceive.isAlive()) {
                            TCPFileReceive.start();
                        }
                        else if(!TCPReplicatedFileReceive.isAlive()) {
                            TCPReplicatedFileReceive.start();
                        }
                        break;
                    }
    ...
    private static void shutdownSend() {
        ...
        while (it.hasNext()) {
            String file = it.next();
            //CHECK IF PREVIOUS NODE STORES ORIGINAL FILE

            if (!file.contains(".log")) {
                int index = file.indexOf(".");
                String fileName = file.substring(0,index);
                //TODO:
                String locationLine = null; //linux
                try {
                    locationLine = Files.readAllLines(Paths.get(fileLocation + "/" +fileName+".log")).get(1);
                    //System.out.println("Line: " +locationLine);
                } catch (IOException e) {
                    e.printStackTrace();
                }
                //locationLine = Files.readAllLines(Paths.get(fileLocation+"\\"+fileName+".log")).get(1); //windows
                String[] location = locationLine.split(" ", 5);
                //System.out.println("Location: " +location[2]);
                //IF TRUE, GET PREVIOUS NODE OF PREVIOUS NODE, SEND FILE AND LOG TO PREVIOUS NODE OF PREVIOUS NODE
                try {
                    if (hashCode(location[2]) == previous) {
                        System.out.println("Previous node has original file");
                        System.out.println("Get previousID of previous node");
                        String previousOfPreviousID = sendGET("getPreviousNode/" + previousNodeID, stringServerAddress);
                        //GET IP OF PREVIOUS NODE OF PREVIOUS NODE, USING REST
                        System.out.println("Get IP of previous node of previous node");
                        String IP = sendGET("getIP/" + previousOfPreviousID, stringServerAddress);
                        System.out.println("Sending " +fileName+ " and its log to " +IP);
                        ...
                        //ELSE, SEND FILE AND LOG TO PREVIOUS NODE
                    } else {
    ...
    public static void update(int id, InetAddress hostAddress) {
        ...
        //First client joins the network
        if (currentNodeID == nextNodeID && currentNodeID == previousNodeID) {
            previousNodeID = id;
            nextNodeID = id;
        } else {
            //Second client joins the network
            ...
            //Current node at end of ring topology
            else if (currentNodeID < id && nextNodeID < id && currentNodeID > nextNodeID) {
                nextNodeID = id;
                sendUnicast("I [" + currentNodeID + "], have put you as my nextNode", hostAddress);
            }
            //Current node at start of ring topology
        ...
        //Check if replication already happened
        if (!runningRep && amountOtherNodes > 0) {
            runningRep = true;
    ...
    public static void removeReplicatedFile(String message) {
        ...
        String path1 = path+"/receivedFiles/"+message; //For linux not the log file path
        String path2 = path+"/receivedFiles/"+fileName1; //For linux for log file path
        try {
